...
/* Local fields */
static uint8_t uart_rx_dma_buff[DMA_RX_BUFFER_SIZE];
static uint8_t uart_tx_dma_buff[DMA_TX_BUFFER_SIZE];
...
static void _usart1_interrupt_idle_set(void *fp);
...
static void _usart1_dma_configure() {
	...
	HAL_UART_Receive_DMA(&huart1, uart_rx_dma_buff, DMA_RX_BUFFER_SIZE);
...
void USART1_IRQHandler() {
	if (__HAL_UART_GET_FLAG(&huart1, USART_FLAG_IDLE) == SET) {
		...
		if (uart_idle_detected) {
			...
			uart_idle_detected(uart_rx_dma_buff,
			DMA_RX_BUFFER_SIZE - hdma_usart1_rx.Instance->NDTR,
			DMA_RX_BUFFER_SIZE);
		}
	}
