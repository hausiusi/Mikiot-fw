...
#define prf_func_exec_time_get(_fn_)                ({prf_start_measurement(); _fn_; uint32_t _fet = prf_end_measurement(); _fet;})
#define prf_func_exec_time_log(_fn_, delay)         ({ debug_p("Measuring execution time for function '%s'\n", #_fn_);\
                                                     uint32_t __fet = prf_func_exec_time_get(_fn_);\
